...
class TransactionContextExample {
    public static void main(String[] args) throws SQLException {
        ...
        var producer = new ConnectionAwareDatabaseProducer();
        try (var connection = dataSource.getConnection()) {
            // Everything below happens in one transaction, so it is committed (or rolled back) as a whole
            connection.setAutoCommit(false);

            // Execute business query - insert new customer to the business table
            var businessQuery = "insert into customer(id, email, name) values(1, 'john.doe@example.com', 'John Doe')";
            try (var statement = connection.createStatement()) {
                statement.execute(businessQuery);
            }
            ...
            System.out.println("The user has been inserted and the message has been sent to the queue");
        }

        // -------------------------------------------------------------------------------------------
        // Create consumer and try to read message from the queue, process it and delete
        var consumer = new ConnectionAwareDatabaseConsumer();
        try (var connection = dataSource.getConnection()) {
            connection.setAutoCommit(false);
            ...
            // Please note that the ConnectionAware* methods take the connection as the first argument. This is different from
            // the regular Producer/Consumer
            var message = consumer.receive(connection, queue);
            if (message != null) {
                // the message from the queue was received, we can emulate the heavy calculation and update the business table
                var heavyData = "Large and heavy calculated data"; // in the real application this will be more complex of course
                var businessQuery = "update customer set additional_info = '" + heavyData + "' where id = 1";
                try (var statement = connection.createStatement()) {
                    statement.execute(businessQuery);
                }
                ...
                System.out.println("The message has been received, the user has been updated and the message has been deleted from the queue");
            }
        }
    }
}
