...
class DeduplicationExample {
    ...
    private static final MetaField<Integer> USER_ID = MetaField.ofInt("user_id", FieldOption.ALL_LIVE_UNIQUE);

    public static void main(String[] args) {
        // Define queue with name `test_queue`, varchar type as data storage and metadata
        var queue = Queue.of("test_queue", PredefinedDataTypes.String, Metadata.of(USER_ID));
        ...
        for (int index = 1; index <= 10; index++) {
            var userId = index % 5;
            var message = new SendMessage<>("Message " + index + ", userId=" + userId, MetaValues.of(USER_ID.value(userId)));
        ...
        producer.send(
                ...
                new SendRequest<>(
                        messagesToSend,
                        SendOptions.builder().deduplicationMode(DeduplicationMode.IGNORE_DUPLICATE).build()
                )
        );

        System.out.println("------------------------------------------------------");
        System.out.println("Sent messages (" + messagesToSend.size() + "): ");
        messagesToSend.forEach(message -> System.out.println("  " + message));
        ...
        var messages = consumer.receive(queue, 100);
        System.out.println("------------------------------------------------------");
