...
interface Inspector {
     ...
     *
     * This method works as if by invoking the two-argument overload with [CountOptions.DEFAULT], which leaves the
     ...
     */
    fun count(queue: Queue<*>): Messages = count(queue, CountOptions.DEFAULT)
     ...
     * @return a [Messages] instance with per-state counts
     */
    fun count(queue: Queue<*>, options: CountOptions): Messages
     ...
     *
     * This method works as if by invoking the four-argument overload with [DistinctValuesOptions.DEFAULT], which
     ...
     */
    fun <V> distinctValues(queue: Queue<*>, metaField: MetaField<V>, limit: Int): Map<V?, Long> =
        distinctValues(queue, metaField, limit, DistinctValuesOptions.DEFAULT)
