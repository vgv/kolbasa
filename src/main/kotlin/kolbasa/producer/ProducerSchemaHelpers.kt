...
internal object ProducerSchemaHelpers {
    ...
    fun generateInsertPreparedQuery(
        ...
        // deduplication
        if (deduplicationMode == DeduplicationMode.IGNORE_DUPLICATE || deduplicationMode == DeduplicationMode.IGNORE_DUPLICATES) {
        ...
        val onConflictStr = if (deduplicationMode == DeduplicationMode.IGNORE_DUPLICATE || deduplicationMode == DeduplicationMode.IGNORE_DUPLICATES) {
            "on conflict do nothing"
        } else {
            ""
        }
        val returningColumns = if (deduplicationMode == DeduplicationMode.IGNORE_DUPLICATE || deduplicationMode == DeduplicationMode.IGNORE_DUPLICATES) {
