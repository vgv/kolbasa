...
internal object ProducerSchemaHelpers {
    ...
    fun <Data> fillInsertPreparedQuery(
        ...
        val delays = Array(request.data.size) { index ->
            ...
            val delay = QueueHelpers.calculateDelay(queue.options, producerOptions, request.options, item.options)
        ...
        val attempts = Array(request.data.size) { index ->
            val item = request.data[index]
            QueueHelpers.calculateAttempts(queue.options, producerOptions, request.options, item.options)
        }
