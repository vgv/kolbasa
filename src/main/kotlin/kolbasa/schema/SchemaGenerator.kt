...
internal object SchemaGenerator {
    ...
    internal fun generateTableSchema(queue: Queue<*, *>, existingTable: Table?, idRange: IdRange): Schema {
        ...
        fixIdentity(queue, existingTable, mutableSchema)
    ...
    private fun forTable(queue: Queue<*, *>, existingTable: Table?, mutableSchema: MutableSchema, idRange: IdRange) {
        val createTableStatement = """
            create table if not exists ${queue.dbTableName}(
                ${Const.ID_COLUMN_NAME} bigint generated by default as identity (minvalue ${idRange.start} maxvalue ${idRange.end} cache ${idRange.cache} cycle) primary key,
    ...
    private fun fixIdentity(queue: Queue<*, *>, existingTable: Table?, mutableSchema: MutableSchema) {
        // TODO delete after few releases
        // alter table q_kotlin alter id set generated by default;
        val alterIdentity =
            "alter table ${queue.dbTableName} alter ${Const.ID_COLUMN_NAME} set generated by default"

        mutableSchema.allTables += alterIdentity
        mutableSchema.requiredTables += alterIdentity
    }
