...
internal object QueueHelpers {
    ...
    fun calculateDelay(queueOptions: QueueOptions?, sendOptions: SendOptions?): Duration? {
        var delay = queueOptions?.defaultDelay
        if (sendOptions != null) {
            if (sendOptions.delay !== QueueOptions.DELAY_NOT_SET) {
    ...
    fun calculateAttempts(queueOptions: QueueOptions?, sendOptions: SendOptions?): Int {
        var attempts = queueOptions?.defaultAttempts ?: QueueOptions.DEFAULT_ATTEMPTS
        if (sendOptions != null) {
            if (sendOptions.attempts != QueueOptions.ATTEMPTS_NOT_SET) {
                attempts = sendOptions.attempts
            }
        }
